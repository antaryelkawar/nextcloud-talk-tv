...
- [x] Implement a prototype for **DOM Overlay** video rendering.
- [ ] Benchmark FPS and CPU usage for DOM Overlay on simulated TV environment.
- [x] **Decision Document:** Finalize and document the chosen strategy in `docs/design_decisions/video_strategy.md`.

## 4. Core Bridge Implementation
- [x] Implement `src/bridge/observer.ts` using the Subscription/Observer pattern.
- [x] **TDD:** Write test to ensure SolidJS signal updates trigger bridge callbacks.
- [ ] **Performance Test:** Verify bridge latency is below the 16ms budget.

## 5. Asset Pipeline
- [x] Create `scripts/convert_assets.ts`.
- [ ] Update pipeline to handle SVG logos and Talk-specific icons.
- [x] Test pipeline with a sample set of assets.
